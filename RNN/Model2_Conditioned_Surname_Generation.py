...
# 获取用户输入并验证
while True:
    student_id_last_digit = input("请输入学号尾数（0-9）：")
    if student_id_last_digit.isdigit() and 0 <= int(student_id_last_digit) <= 9:
        student_id_last_digit = int(student_id_last_digit)
        break
    else:
        print("输入无效，请重新输入0-9的数字。")
...
# 获取目标国家
target_nationality = country_mapping[student_id_last_digit]

# 获取国家索引
try:
    nationality_index = vectorizer.nationality_vocab.lookup_token(target_nationality)
except KeyError:
    print(f"错误：数据集中找不到'{target_nationality}'国家的样本")
    exit(1)
...
# 打印结果
print(f"\n生成的 {target_nationality} 风格姓氏：")
for i, surname in enumerate(decoded_surnames, 1):
    print(f"{i}. {surname}")